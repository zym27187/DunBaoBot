...
API_KEY = 'SnKvkrPQVrgFuwTxLu3jZml3'   #你的API Key
SECRET_KEY = 'MtQXvppTTeVfSRpKZ2UEnEPMjkVjHU3g'#你的Secret Key
...
async def setu_score(bot,ev: CQEvent):
    ...
    if reply is False:
        print(score)
        if score <= 60:
            msg = R.img('硬不起来.jpg').cqcode
            await bot.send(ev,str(MessageSegment.image(url) + f'\n色图评分:{score}\n'+f'{msg}'))
        else:await bot.send(ev,str(MessageSegment.image(url)+f'\n色图评分:{score}'))
    else:
