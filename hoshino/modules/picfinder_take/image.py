...
class SauceNAO():
    ...
    async def get_view(self, sauce) -> str:
        ...
        for sauce in sauces['results']:
            try:
                url = sauce['data']['ext_urls'][0].replace("\\", "").strip() if 'ext_urls' in sauce[
                    'data'] else "no link"
                ...
                if THUMB_ON:
                    try:
                        thumbnail_image = str(
                            MessageSegment.image(pic2b64(ats_pic(Image.open(BytesIO(await get_pic(thumbnail_url)))))))
                    except Exception as e:
                        print(format_exc())
        ...
        for data in all_data[1:self.num + 1]:
            try:
                ...
                if not data.xpath('.//div[@class="detail-box gray-link"]/h6'):
                    data2 = data.xpath('.//div[@class="external"]')[0] if data.xpath(
                        './/div[@class="external"]') else data
        ...
        for line in data:
            if THUMB_ON:
                try:
                    ...
                    thumbnail_image = str(MessageSegment.image(pic2b64(
                        ats_pic(Image.open(BytesIO(self.scraper.get(line[2], timeout=20, proxies=proxies).content))))))
                except Exception as e:
                    print(format_exc())
                    thumbnail_image = "[预览图下载失败]"
            else:
